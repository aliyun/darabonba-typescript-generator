...
const {
  _name, _string, _type, _subModelName, _vid, _moduleName, _fieldName, 
...
Client.main(process.argv.slice(2));`);
  ...
  eachImport(imports, usedModels, level) {
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        if (!moduleDir) {
          this.emit(`import ${_moduleName(aliasId)}`, level);
          const models = [...usedModels.get(aliasId)];
          if (models.length > 0) {
            this.emit(`, * as $${_moduleName(aliasId)}`, level);
          }
  ...
  visitType(ast, level) {
    if (ast.type === 'array') {
      ...
      this.emit(`$${_moduleName(moduleId.lexeme)}.`);
      this.emit(_subModelName(rest.map((item) => {
        return item.lexeme;
      }).join('.')));
    } else if (ast.type === 'subModel') {
      const [moduleId, ...rest] = ast.path;
      this.emit(_subModelName([_moduleName(moduleId.lexeme), ...rest.map((item) => {
        return _name(item);
      })].join('.')));
      ...
      if (ast.moduleName) {
        this.emit(`$${_moduleName(ast.moduleName)}.`);
      }
      this.emit(`${ast.name}`);
    } else {
  ...
  visitFieldType(value, level, modelName, fieldName) {
    if (value.type === 'modelBody') {
      ...
      this.emit(`$${_moduleName(moduleId.lexeme)}.${_subModelName(models.map((item) => item.lexeme).join('.'))}`);
      ...
      this.emit(`${_moduleName(_type(value.fieldType.lexeme))}`);
  ...
  visitModelBody(ast, level, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.visitComments(comments, level);
      this.emit(`${_fieldName(node.fieldName)}${node.required ? '' : '?'}: `, level);
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (nameAttr) {
        this.emit(`${_fieldName(node.fieldName)}: '${_string(nameAttr.attrValue)}',\n`, level + 2);
      } else {
        this.emit(`${_fieldName(node.fieldName)}: '${node.fieldName.lexeme}',\n`, level + 2);
      }
    }
    this.emit(`};\n`, level + 1);
    this.emit(`}\n`, level);
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      const node = ast.nodes[i];
      this.emit(`${_fieldName(node.fieldName)}: `, level + 2);
  ...
  visitObjectField(ast, level) {
    ...
    if (ast.type === 'objectField') {
      var key = _fieldName(ast.fieldName);
      this.emit(`${key}: `, level);
  ...
  visitConstruct(ast, level) {
    ...
    this.emit('new ');
    this.emit(`${_moduleName(ast.aliasId.lexeme)}`);
    this.visitArgs(ast.args, level);
  }
  ...
  visitConstructModel(ast, level) {
    ...
    if (ast.aliasId.isModule) {
      ...
      this.emit(`new $${_moduleName(moduleName)}.`);

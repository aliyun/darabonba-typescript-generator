  ...
  eachImport(imports, usedModels, level) {
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        const tsPkg = teaFile.releases && teaFile.releases.ts;
        if (!tsPkg) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType) {
        if (node.fieldValue.fieldType === 'array') {
          const itemType = node.fieldValue.fieldItemType;
          if (node.fieldValue.itemType) {
            this.emit(`${_type(_subModelName(node.fieldValue.itemType))}[]`);
          } else if (itemType.type === 'map') {
            this.emit(`{ [key: ${itemType.keyType.lexeme}]: ${itemType.valueType.lexeme} }[]`);
          } else {
            this.emit(`${_type(node.fieldValue.fieldItemType.lexeme)}[]`);
          }
        } else if (node.fieldValue.fieldType === 'map') {
          this.emit(`{ [key: ${node.fieldValue.keyType.lexeme}]: ${node.fieldValue.valueType.lexeme} }`);
          ...
          this.emit(`$${moduleId.lexeme}.${models.map((item) => item.lexeme).join('.')}`);
        } else if (node.fieldValue.fieldType.type) {
          this.emit(`${_type(node.fieldValue.fieldType.lexeme)}`);
        } else if (node.fieldValue.fieldType.idType === 'model') {
          this.emit(`${_type(node.fieldValue.fieldType.lexeme)}`);
        } else if (node.fieldValue.fieldType.idType === 'builtin_model') {
          this.emit(`${_type(node.fieldValue.fieldType.lexeme)}`);
        }
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if (node.fieldValue.fieldType) {
        if (node.fieldValue.fieldType === 'array') {
          ...
          const itemType = node.fieldValue.fieldItemType;
          if (itemType.tag === DSL.Tag.Tag.ID) { // id
            this.emit(itemType.lexeme);
          } else if (itemType.type === 'modelBody') {
            this.emit(_subModelName([modelName, _name(node.fieldName)].join('.')));
          } else if (itemType.type === 'map') {
            this.emit(`{ 'type': 'map', 'keyType': '${itemType.keyType.lexeme}', 'valueType': '${itemType.valueType.lexeme}' }`);
          } else if (itemType.tag === DSL.Tag.Tag.TYPE) {
            // basic type
            this.emit(`'${itemType.lexeme}'`);
          } else {
          ...
          this.emit(`{ 'type': 'map', 'keyType': '${fieldValue.keyType.lexeme}', 'valueType': '${fieldValue.valueType.lexeme}' }`);
        } else if (node.fieldValue.fieldType === 'object') {

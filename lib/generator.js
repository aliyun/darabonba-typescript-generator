...
Client.main(process.argv.slice(2));`);
  ...
  visitConstructModel(ast, level) {
    ...
    if (ast.aliasId.isModule) {
      ...
      this.emit(`new $${moduleName}.${modelName}`);
  ...
  moduleBefore(ast) {
    ...
    if(!this.exportGen && Object.keys(this.exports).length > 0) {
      ...
      Object.keys(this.exports).map(aliasId => {
        this.emit(`export * as $${aliasId} from '${this.exports[aliasId].replace(/(\.tea)$|(\.spec)$|(\.dara)$/gi, '')}';\n`);
        this.emit(`export { default as ${aliasId} } from '${this.exports[aliasId].replace(/(\.tea)$|(\.spec)$|(\.dara)$/gi, '')}';\n`);
      });
      this.emit(`\n`);
    }
  }

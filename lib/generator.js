...
Client.main(process.argv.slice(2));`);
  ...
  visitExpr(ast, level) {
    if (ast.type === 'boolean') {
      ...
      this.emit('!');
      const isGroup = this.checkGroupOp(ast.expr);
      if(isGroup) {
        this.emit('(');
      }
      this.visitExpr(ast.expr, level);
  ...
  checkGroupOp(ast){
    if(ast.type !== 'call' || ast.left.type === 'static_call' ) {
      return false;
    }
    ...
    if(ast.left.type === 'instance_call') {
      const method = _name(ast.left.propertyPath[0]);
      if (ast.builtinModule === '$String' && method === 'equals') {
        return true;
      }
    }
  ...
  visitTry(ast, level) {
    ...
    if (ast.catchBlocks && ast.catchBlocks.length > 0) {
      ...
      ast.catchBlocks.forEach((catchBlock, index) => {
        if (!catchBlock.id) {
          return;
        }
        ...
        const returnStmt = catchBlock.catchStmts.stmts.find(stmt => stmt.type === 'return' || stmt.type === 'yield');
        if (!returnStmt && (ast.catchBlocks.length - 1) !== index) {
          this.emit('return;\n', level + 1);
        }
        if (catchBlock.id.type) {
          level--;

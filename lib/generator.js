...
const {
  ...
  _isBinaryOp, _escape,
  REQUEST, RESPONSE, CORE, _upperFirst
} = require('./helper');
...
Client.main(process.argv.slice(2));`);
  ...
  eachImport(imports, usedModels, innerModule, level) {
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        if(inner) {
          const innerPath = pkg.exports[inner].replace(/(\.tea)$|(\.spec)$|(\.dara)$/gi, '');
          
          const pkgPath = path.join(pkgName, 'src', innerPath);
          this.emit(`'${pkgPath}'`);
        } else {
          this.emit(`'${pkgName}'`);
        }
  ...
  visitExtendOn(extendOn, type = 'model') {
    ...
    if (extendOn.type === 'moduleModel') {
      ...
      this.emit(`${_upperFirst(_name(extendOn))}`);
    }
  ...
  visitEcxceptionConstrutor(ast, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      node = ast.nodes[i];
      this.emit(`this.${_escape(_name(node.fieldName))} = map.${_escape(_name(node.fieldName))};\n`, level);
    }
  }

  visitException(exceptionBody, exceptionName, extendOn, level) {
    this.emit(`export class ${exceptionName} extends `, level);
  ...
  visitConstruct(ast, level) {
    ...
    this.emit('new ');
    this.emit(`${_type(ast.aliasId.lexeme)}`);
    this.visitArgs(ast.args, level);
  }
  ...
  moduleBefore(ast) {
    ...
    if (ast.usedTypes.has('writable')) {
      str += ', Writable';
    }

    if (str) {
      this.emit(`import { ${str} } from 'stream';\n`);
    }
    this.emit(`import * as $dara from '@darabonba/typescript';\n`);
    this.typedefImport.forEach(type => {
      this.emit(`import * as ${type} from '${type}';\n`);
    });
    this.emit(`\n`);
  }

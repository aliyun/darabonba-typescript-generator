...
const  { _type, _vid, _name, CORE } = require('./helper');
...
class Bytes extends Builtin {
  ...
  toString(ast, type = '"utf8"'){ 
    this.getInstanceName(ast);
    this.generator.emit(`.toString(${type})`);
  }

  toHex(ast){ 
    this.toString(ast, '"hex"');
  }

  toBase64(ast){ 
    this.toString(ast, '"base64"');
  }
